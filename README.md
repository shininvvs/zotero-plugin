...
경고 알림은 시간이 지나도 사라지지 않고, 아무 곳이나 클릭하면 닫힙니다.
놓치면 알릴 이유가 없어지기 때문인데, 모달 대화상자와 달리 다른 작업을
막지는 않습니다.

Zotero 의 알림 API 에는 버튼을 넣을 수단이 없어서(옵션이 `window` 와
`closeOnClick` 뿐입니다) 창 전체가 클릭 영역이며, 그 사실을 마지막 줄에
글로 적어 둡니다.
...
OpenReview 나 arXiv 에서 가져온 학회 논문은 이 필드가 비어 있는 경우가
많습니다. **협의 명**은 채워져 있어도 **의사록**은 별개 필드라 따로 넣어야
합니다.
